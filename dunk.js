...
function createDice(number) {
  ...
  for (const dotPosition of dotPositionMatrix[number]) {
    ...
    dot.style.setProperty("--top", dotPosition[0] - 8 + "%");
    dot.style.setProperty("--left", dotPosition[1] - 6 + "%");
...
function initializeGame() {
  ...
  btnRollDice.addEventListener("click", () => {
    ...
    setTimeout(() => {
      ...
      const diceValues = randomizeDice(diceContainer, NUMBER_OF_DICE);
      for (let i = 0; i < diceValues.length; i++) {
        sum += diceValues[i];
      }
      if ((sum == 10, 5)) {
        dunk = true;
        Maze(); // Call CreateKeys() when dunk is true
      } else {
        dunk = false;
      }
